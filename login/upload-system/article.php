...
?>
    ...
    <section class="hero">
        <div class="container">
            <div class="hero-con">
            <?php 
            $sql = "SELECT * FROM tb_article ORDER BY article_date DESC ";
            if (isset($_GET['search'])) {
                $search = $_GET['search'];
                $sql = "SELECT * FROM tb_article WHERE article_topic LIKE '%$search%' ORDER BY article_date DESC ";
            }
$query = $db2->query($sql);
if ($query->num_rows > 0) {
    while($row = $query->fetch_assoc()) {
        $imageURL = 'http://fighter.nstroke.com/uploads/'.$row['article_img'];
        $section = $row['article_topic'];
        $content = $row['article_short'];
        // $imageID = $row['id']; // Assuming 'id' is the primary key of your images table
        $uploadedOn = date('M j, Y', strtotime($row['article_date'])); // Format the date as desired
        $article_id = $row['article_id'];
        
        ?>
        <div class="box-con">
    <div class="hero-img">
        <div class="uploaded-date"><?php echo $uploadedOn ?></div>
        <img src="<?php echo $imageURL ?>" alt="" width="100%" height="100%" class="card-img">
    </div>
    <div class="hero-info">
        <h5><?php echo $section ?></h5>
        <p><?php echo $content ?></p>
        <a href="detail.php?id=<?php echo $article_id ?>" class="btn btn-primary">Read More</a>
    </div>
</div>
...
<?php } ?>
        ...
        <center>
        <div class="add-btn">
                <a href="../login.php" class="btn btn-primary">เพิ่มบทความ</a>
        </div>
        </center>
    </section>
